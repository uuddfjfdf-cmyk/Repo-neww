...
// ✅ DIGANTI: dari EMAIL_USER + EMAIL_PASS (nodemailer) ke RESEND_API_KEY
...
// ── SEND MAGIC LINK ──────────────────────────────
// ✅ DIMODIFIKASI: Ganti nodemailer SMTP dengan Resend API via HTTPS fetch
app.post('/api/auth/magic-link', async (req, res) => {
  try {
    const { email } = req.body;
    if (!email) return respond(res, 400, false, 'Email diperlukan');

    // Cek apakah RESEND_API_KEY sudah di-set via Environment Variables Railway
    if (!process.env.RESEND_API_KEY) {
      console.error('❌ RESEND_API_KEY belum di-set! Magic link tidak bisa dikirim.');
      console.error('   • Lokal  : tambahkan RESEND_API_KEY di file .env');
    ...
    // ── [CCTV] EKSEKUSI KIRIM EMAIL — Resend API via HTTPS fetch ────────────
    try {
      const resendResponse = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from   : 'AuthLab <onboarding@resend.dev>',
          to     : email,
      ...
      // Tangkap response Resend untuk logging & deteksi kegagalan non-exception
      if (!resendResponse.ok) {
        const resendError = await resendResponse.json().catch(() => ({}));
        console.log('');
        console.log('📧 ══════════════════════════════════════════════════ 📧');
        console.log('❌ [MAGIC LINK EMAIL FAILED] — Resend API mengembalikan status error');
        console.log('📧 ══════════════════════════════════════════════════ 📧');
        console.log('📮 Email Tujuan      :', email);
        console.log('🔢 HTTP Status       :', resendResponse.status);
        console.log('💬 Pesan Error       :', resendError.message || resendError.name || JSON.stringify(resendError));
        console.log('📧 ══════════════════════════════════════════════════ 📧');
        console.log('');
      ...
      // ── [CCTV TOTAL] Fetch melempar error (network down, DNS gagal, timeout) ─
      console.log('');
      console.log('📧 ══════════════════════════════════════════════════ 📧');
      console.log('❌ [MAGIC LINK EMAIL FAILED] — Resend API melempar error');
      console.log('📧 ══════════════════════════════════════════════════ 📧');
      console.log('📮 Email Tujuan      :', email);
      console.log('💬 Pesan Error       :', mailErr.message || '(tidak ada pesan)');
      console.log('🔢 Status Kode       :', mailErr.status  || mailErr.code || '(tidak ada)');
      console.log('📧 ══════════════════════════════════════════════════ 📧');
      console.log('');
      return respond(res, 500, false, 'Gagal mengirim email. Coba beberapa saat lagi.');
    }

    console.log(`✅ Magic link berhasil dikirim ke ${email} via Resend API`);
    respond(res, 200, true, 'Magic link berhasil dikirim! Cek inbox email kamu.');
